...
CREATE TABLE run.runs (
  ...
  experiment_id      UUID           NOT NULL REFERENCES experiments.experiments(id)      ON DELETE RESTRICT,
  cluster_id         UUID           NOT NULL REFERENCES run.compute_clusters(id)         ON DELETE RESTRICT,
...
CREATE TABLE run.run_logs (
  ...
  run_id      UUID          NOT NULL REFERENCES run.runs(id) ON DELETE CASCADE,
...
CREATE TABLE run.run_output_artifacts (
  ...
  run_id           UUID         NOT NULL UNIQUE REFERENCES run.runs(id) ON DELETE CASCADE,
